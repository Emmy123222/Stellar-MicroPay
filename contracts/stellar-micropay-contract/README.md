...
### `create_escrow(env: Env, from: Address, to: Address, amount: i128, release_ledger: u32) -> u32`

- **Parameters**:
  - `from: Address` - escrow funder; the amount is pulled from this account into the contract.
  - `to: Address` - escrow beneficiary; the amount is released to them once `release_ledger` is reached.
  - `amount: i128` - escrow amount.
  - `release_ledger: u32` - ledger at or after which the escrow can be claimed.
- **Return value**: zero-based escrow id.
- **Authorization requirements**: `from.require_auth()`.
- **Events emitted**: `(escrow_create, id)` with `(from, to, amount, release_ledger)`.
- **Error conditions**:
  - Panics with `Contract is paused` when the emergency pause is active (#802).
  - Panics with `amount must be positive` when `amount <= 0`.
  - Panics with `release_ledger must be in the future` when `release_ledger <= current ledger`.
  - Propagates token contract transfer failures, including insufficient balance, missing trustline, or token authorization failures.

Escrowed funds are held by the contract until `release_ledger`, then claimed by
the beneficiary (`claim_escrow`) or returned to the funder (`cancel_escrow`).
...
- **Error conditions**: `stream not found`, `unauthorized`, `stream is closed`, `amount must be positive`, or `Contract is paused` when the emergency pause is active (#802).
...
| `3` | Adds `DataKey::PendingAdmin`, `DataKey::PendingAdminExpiry` (two-step admin transfer, #801) and `DataKey::Paused` (emergency pause, #802). Purely additive — no existing struct or key changes shape, so no rewrite of existing entries is required. |
