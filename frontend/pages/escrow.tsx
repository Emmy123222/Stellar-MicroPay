...
export default function EscrowPage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      if (!publicKey) return;
      try {
        const [bal, ledger] = await Promise.all([getXLMBalance(publicKey), getCurrentLedger()]);
        if (cancelled) return;
        setXlmBalance(bal);
  ...
  async function handleLookup() {
    ...
    setActionError(null);
    try {
      const [escrow, ledger] = await Promise.all([getEscrow(publicKey, id), getCurrentLedger()]);
      if (!escrow) {
  ...
  async function handleAction(action: "claim" | "cancel") {
    ...
    try {
      const builder =
        action === "claim" ? buildClaimEscrowTransaction : buildCancelEscrowTransaction;
      ...
      const newEvent: TimelineEvent = {
        ...
        timestamp: Date.now(),
        description:
          action === "claim" ? "Funds released to recipient" : "Funds refunded to sender",
      };
