...
export function MonthlySpendingChart({
  ...
  if (loading && data.length === 0) {
    return <div className="card mb-6 h-[350px] animate-pulse bg-white/[0.03] border-white/10" />;
  }

  return (
    <div className="card mb-6 overflow-hidden">
      <h2 className="font-display text-lg font-semibold text-white mb-6">Monthly Spending (XLM)</h2>
      <div className="h-[250px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={data}
            onClick={(state: any) =>
              state && state.activePayload && onBarClick(state.activePayload[0].payload)
            }
          >
