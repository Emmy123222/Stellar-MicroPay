...
export default function MultiSigFlow({
  ...
  function scheduleLocalReminder(unsignedXDR: string, signers: string[], threshold: number) {
    ...
    const timer = setTimeout(() => {
      ...
      console.log(
        `[Multi-sig Reminder] Signature pending for transaction. Signers: ${signers.join(", ")}, Threshold: ${threshold}`
      );
            ...
            return (
              ...
              >
                ...
                {i < MULTISIG_FLOW_STEPS.length - 1 && (
                  <div
                    className={clsx("w-6 h-px mx-2", isComplete ? "bg-stellar-500" : "bg-white/10")}
                    aria-hidden="true"
                  />
                )}
              </li>
      ...
      >
        ...
        {step === "sign" && (
          <div className="space-y-4">
            ...
            <button
              onClick={handleReset}
              className="text-xs text-slate-500 hover:text-slate-300 w-full text-center transition-colors"
            >
              ← Start over
            </button>
          </div>
        ...
        {step === "share" && (
          <div className="space-y-4">
            <p className="text-slate-300 text-sm">
              Your signature has been added. Share this link with your co-signers so they can sign
            ...
            >
              {copied ? (
                <CheckSmallIcon className="w-4 h-4 text-green-400" />
              ) : (
        ...
        {step === "collect" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-slate-300 text-sm">
                Signatures: <span className="font-bold text-white">{signaturesCollected}</span> /{" "}
                {threshold}
              </p>
              {thresholdMet && (
            ...
            {thresholdMet && (
              <button onClick={() => setStep("submit")} className="btn-primary w-full py-2.5">
                Proceed to Submit →
              </button>
            )}
          </div>
