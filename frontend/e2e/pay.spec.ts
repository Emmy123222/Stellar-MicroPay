...
base.describe("pay page - disconnected", () => {
  ...
  base.test(
    "visiting /pay with valid SEP-0007 params prompts for wallet connect and shows payment header",
    async ({ page }) => {
  ...
  base.test(
    "visiting /pay with incomplete params shows malformed payment link error",
    async ({ page }) => {
      ...
      await expect(
        page.getByText("The payment link data is incomplete or malformed.")
      ).toBeVisible();
  ...
  base.test(
    "visiting /pay with invalid expiry timestamp shows invalid expiry error",
    async ({ page }) => {
...
authenticatedTest.describe("pay page - authenticated", () => {
  authenticatedTest(
    "visiting /pay with valid SEP-0007 params pre-fills payment form inputs",
    async ({ page }) => {
      await page.goto(`/pay?to=${VALID_DESTINATION}&amount=${VALID_AMOUNT}&memo=${VALID_MEMO}`);
