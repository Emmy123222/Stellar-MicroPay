...
const mockValidXDR = 'AAAAAgAAAAD8AAAAAAAAAAQAAAAAAAAAAAAAAAABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABAAAAAEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAAAAAAAABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABAAAAAAAAAAAAAAAAAAAA';
...
authenticatedTest.describe('multi-sig-sign page', () => {
  authenticatedTest('importing a valid XDR shows the pending signature state', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Sign Multi-Signature Transaction' })).toBeVisible({ timeout: 10_000 });
