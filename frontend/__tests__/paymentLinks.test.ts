...
describe("shareable payment link urls", () => {
  ...
  it("parses the network query param onto the payload", () => {
    expect(
      parsePaymentLinkQuery({
        to: "GABCDEF",
        amount: "10",
  ...
  it("parses the network from legacy base64 data links", () => {
    const data = btoa(JSON.stringify({ ...PAYLOAD, network: "testnet" }));
    expect(parsePaymentLinkQuery({ data })).toEqual({
      ok: true,
      payload: { ...PAYLOAD, network: "testnet", validUntil: null },
    });
  });

  it("rejects links with an unknown network value", () => {
    expect(
      parsePaymentLinkQuery({ to: "GABCDEF", amount: "10", network: "foo" }),
    ).toEqual({ ok: false, reason: "invalid-network" });

    expect(
      parsePaymentLinkQuery({ to: "GABCDEF", amount: "10", network: "PUBLIC" }),
    ).toEqual({ ok: false, reason: "invalid-network" });
  });
