...
describe("sep0007 URI parsing", () => {
  describe("Generates valid SEP-0007 URIs", () => {
    ...
    it("parses a URI with destination, amount, and memo", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&amount=100&memo=TestPayment";
      const result = parseStellarURI(uri);
    ...
    it("parses a web+stellar:pay URI", () => {
      const uri =
        "web+stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&amount=50";
      const result = parseStellarURI(uri);
    ...
    it("parses stellarmicropay:// deep link with to parameter", () => {
      const uri =
        "stellarmicropay://pay?to=GABC123456789012345678901234567890123456789012345678&amount=25";
      const result = parseStellarURI(uri);
  ...
  describe("Parses valid URI back into operation params", () => {
    it("extracts all optional parameters correctly", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&amount=100&asset_code=USDC&asset_issuer=GDEF456789012345678901234567890123456789012345678901&memo=Invoice123&memo_type=MEMO_TEXT&msg=Payment%20for%20services";
      const result = parseStellarURI(uri);
  ...
  describe("Rejects malformed or unsupported URIs", () => {
    it("rejects URI without stellar: or web+stellar: scheme", () => {
      const uri =
        "http://example.com?destination=GABC123456789012345678901234567890123456789012345678";
      const result = parseStellarURI(uri);
    ...
    it("rejects URI with invalid amount", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&amount=-50";
      const result = parseStellarURI(uri);
    ...
    it("rejects URI with non-numeric amount", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&amount=abc";
      const result = parseStellarURI(uri);
    ...
    it("rejects URI with asset_code but missing asset_issuer", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&asset_code=USDC";
      const result = parseStellarURI(uri);
    ...
    it("allows XLM asset_code without asset_issuer", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&asset_code=XLM&amount=100";
      const result = parseStellarURI(uri);
    ...
    it("handles malformed query parameters gracefully", () => {
      const uri =
        "stellar:pay?destination=GABC123456789012345678901234567890123456789012345678&&&amount=100";
      const result = parseStellarURI(uri);
